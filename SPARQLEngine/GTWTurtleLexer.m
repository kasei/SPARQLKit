...
- (GTWTurtleToken*) newTokenOfType: (GTWTurtleTokenType) type withArgs: (NSArray*) args {
	NSUInteger start	= self.startCharacter;
	NSUInteger length	= self.character - start;
	NSRange range	= { .location = start, .length = length };
	return [[GTWTurtleToken alloc] initTokenOfType:type withArguments:args fromRange:range];
}
...
- (GTWTurtleToken*) newPName: (NSArray*) pname {
	return [self newTokenOfType:PREFIXNAME withArgs:pname];
}

- (GTWTurtleToken*) getPName {
...
- (GTWTurtleToken*) getDoubleLiteral {
	//	NSString* c	= [self _peekChar];
	[self _getCharSafe: @"\""];
	if ([self.buffer hasPrefix: @"\"\""]) {
...
- (GTWTurtleToken*) getSingleLiteral {
	//	NSString* c	= [self _peekChar];
	[self _getCharSafe: @"'"];
	if ([self.buffer hasPrefix: @"''"]) {
		if (![self _readWord:@"''"]) {
...
- (GTWTurtleToken*) getIRIRefOrRelational {
...
- (void) _lookahead {
    if (!self.lookahead) {
        GTWTurtleToken* t   = [self _getToken];
        self.lookahead      = t;
    }
}
...
- (GTWTurtleToken*) getToken {
    [self _lookahead];
    GTWTurtleToken* t   = self.lookahead;
    self.lookahead      = nil;
    return t;
}

- (GTWTurtleToken*) _getToken {
...
// we're ignoring comment tokens, but we could return them here instead of falling through to the 'next':
			GTWTurtleToken* t	= [self _getComment];
			if (self.comments) {
				return t;
			} else {
				goto NEXT;
...
//			NSLog(@"getting pname");
			GTWTurtleToken* t	= [self getPName];
			if (t) {
				return t;
			}
		}
